...
## 18. Revision-5 owner runtime and dependency closure

This section is inside the Kernel artifact span and supplies owner-runtime and
dependency-closure rules used by the sole declarations in sections 19 and 20. The
bundle instance below the sentinel is generated metadata outside the artifact span.

### 18.2 Exact dependency closure### 18.2 Exact dependency closure
...
## 19. Revision-5 generic ABI, ingress and recovery

This section is inside the Kernel artifact span and owns the generic runtime,
ingress capability, persistence and recovery ABI used by section 20. It has no
legacy structural alias or fallback.
...
### 19.3 Four affine incoming capabilities### 19.3 Four affine incoming capabilities
...
```ts
interface RemoteIngressReservationRequestV2 {
...
Before calling `reserveRemoteIngress`, Kernel authenticates and consumes the live
process-only attempt, defensively copies and caps the exact manifest-core bytes,
requires restricted-JCS byte equality, recomputes `exactManifestDigest`, and checks
the stable key and declared length mirrors. Invalid signatures, principals,
attempts, canonical bytes, digests or mirrors call the persistence port zero times.
The adapter fsyncs that exact copy without re-encoding and rechecks length, digest
and canonical byte equality on reopen. Neither the request nor any durable record
contains an attempt or connection id.
...
The closed Revision-5 limit object has 73 fields. Dependency ingress permits at
most 8192 objects/536870912 bytes per Project epoch and 512 objects/134217728 bytes
per stable source member.
...
The registry contains exactly 129 raw-UTF-8 sorted unique domains. It includes
`convax.signed-wrapper-subject/2` and adds only
`convax.document-shard-reset-authorization-carrier/2` at the terminal delta. The
Canvas genesis carrier remains a binary format literal, not a domain. Protocol
major stays `2` only under the early gate in section 20.6.
...
winner; or if limits/registry counts are not 73/129.

## 20. Revision-5 dependency, base-reference and transfer contract

This is the sole Revision-5 declaration of dependency, base-reference and stable-transfer identities. Earlier structural variants have no decoder or fallback.
...
Kernel alone mints the process-only attempt after exact handshake, channel-open and current principal verification. It privately binds the live connection object, channel-open digest, protocol digest and authenticated stable identity. It has no serializer, structural admission or durable representation. `projectEpoch: Uint64V2`, string connection ids and caller-built attempts reject.
...
Revision 5 replaces the two object-count field names, changes the source-member
...
Revision 5 adds exactly one digest domain:
...
Revision 5 is falsified if a valid Id128 Project epoch cannot round-trip through
...
### 20.6 Exact-protocol early decode gate

Protocol major remains `"2"` only under this mandatory early gate: Peer handshake
and channel-open validate the exact Revision-5 `protocolDigest` before constructing
a body decoder. `CVXPEER2` framing may decode only bounded `PeerMessageCoreV2`,
then requires `core.protocolDigest` equality and signature before parsing body
bytes. Durable objects compare stored protocolDigest before every manifest, carrier
or owner decoder. If any runtime dispatches a `/2` body before exact digest
equality, this authority MUST rotate to protocol major `"3"` and be re-signed.
There is no compatibility negotiation or dual decode.

<!-- BEGIN PROTOCOL_SCHEMA_BUNDLE_V2_INSTANCE -->

## 21. Instantiated Revision 5 ProtocolSchemaBundleV2

This is the sole generated Revision-5 bundle instance. It is outside the Kernel
artifact span, so the Kernel artifact digest cannot contain itself.

### 21.1 Exact source and artifact identities

- canvas-schema artifact bytes: 162839; ordinary SHA-256
  `860d598dca435aa27f7bfd5b588219ebf0193f89f7d2b2c90d519f33de8f3729`;
- collaboration-kernel artifact bytes: 88347; ordinary SHA-256
  `0f0c4602d7612920f3d0b8880ca53b2dd867cd04a64d86577f63448af84fcc78`;
- control-plane artifact bytes: 212607; ordinary SHA-256
  `d3cfe7b1ca626354f05c25c5bf6ab37553eb762e593e350baa9b1bd0f63b8b1b`;
- project-persistence artifact bytes: 197446; ordinary SHA-256
  `59b35653fb0fe01d8acea884e4dc08ad019afcf16ce29f37914215c31c5aad94`;

The exact restricted-JCS artifact manifest is:

```json
[{"artifactDigest":"d6848565a98cea7b106e7d0e653a5c9339f534fae88368d90a642c481f4d0af5","format":"convax.canvas-protocol-schema/2","name":"canvas-schema"},{"artifactDigest":"26657de0f0493e54c71c3c955d70694e093cc43eb60d40e752e8459ce8989237","format":"convax.collaboration-kernel-protocol-schema/2","name":"collaboration-kernel"},{"artifactDigest":"36786c270d4737d1662771d32826a580d93694e24eebc95e2dd57e7bfa53e4b0","format":"convax.control-plane-protocol-schema/2","name":"control-plane"},{"artifactDigest":"e7a291e60122294bb323cd5701e0d97af50e86012661b7c7e50ab2389a67ccbc","format":"convax.project-persistence-protocol-schema/2","name":"project-persistence"}]
```
...
88347-byte prefix through the LF before the sentinel. The resulting
digests are:

```text

```
...
not a digest domain. Revision 5 adds only
`convax.document-shard-reset-authorization-carrier/2`.
...
The limits digest is over the exact 73-field Revision-5 value. Yjs remains exactly
...
wrapper. Revision 4, unsigned overwrite candidates, old registry sizes and old
epoch/dependency/reset/ingress field spellings fail before decoder, owner runtime,
persistence or Peer channel construction.
