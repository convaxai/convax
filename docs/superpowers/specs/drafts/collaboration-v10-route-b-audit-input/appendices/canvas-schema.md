...
`CVXCGP02` semantics. It imports only exact shared Kernel and Control types named
by the active manifest. The words MUST, MUST NOT, SHOULD and MAY are normative.
...
Canvas consumes the transient session coordinator through the real public Kernel
export and uses a consumer-local name. Canvas does not redeclare or structurally
alias the coordinator:

```ts
import type {
  SessionUndoCoordinatorV2 as KernelOwnedSessionUndoCoordinatorV2,
...
The terminal Canvas identity and genesis-core shapes are declared exactly once in
section 24.4. Earlier sections do not provide a structural alias or fallback.
...
## 25. Revision-5 Canvas genesis-proof carrier

This terminal section is the sole Revision-5 Canvas authority for the `CVXCGP02`
carrier, Canvas-owned genesis-state verification and the boundary between validated
G identity and Project reset processing. It replaces every conflicting carrier
statement inherited from source evidence. No earlier draft, unavailable Revision-4
byte, Revision-5 proposal or implementation supplies a fallback decoder.

### Exact binary envelope
...
The old `authorityDependencyCoreDigests` field and the short aliases
...
### Canvas-owned validation

The complete resolver first validates section structure, ordinary hashes and the
historical author closure using the exact selected protocol artifacts. It then
passes the Canvas-owned checkpoint/update/state material to the exact selected
Canvas runtime.
...
This is an immutable process result of the selected artifact call. It is not
serializable authority, a causal dependency, a Project mutation permit or a
replacement for exact checkpoint bytes.

### F identity boundary

The three F observations have distinct authority:

```text
1. C.predecessorRouteFrameDigest:
   untrusted lookup hint from the structurally valid reset carrier.

2. Validated G CanvasIdentity.projectIndexRouteDependencyFrameDigest:
   independently authenticated genesis identity returned only after complete
   CVXCGP02 and Canvas validation.

3. Kernel OwnerResolvedBaseReferences phase view:
   exact accepted-base F envelope bytes.
```

The C hint and validated G identity must be byte-equal before any base-closure
lookup. The phase view must then return exact F bytes for that same digest.

Canvas supplies only observation 2. It must never label it an untrusted hint, read
F from C, open a base-reference phase, decide whether F belongs to the Project
authored base, or treat the digest as sufficient frame evidence.

The portable Canvas Owner ref remains exactly:
...
F is not a Canvas Owner dependency and does not enter this ref.

### Project reset boundary

`DocumentShardResetCurrentRouteFactsV2` remains Control-owned and has exactly twelve
fields, including `currentStagedResetAuthorizationCarrierDigest`. Canvas does not
declare, construct, decode or consume that DTO.

For reset, Project/Control must require:
...
at F13 step 12 after candidate-binding validation.

Canvas verification of G completes before Project reset candidate mutation. The
Project candidate remains an exact unmutated clone during initial and final F13.
After A4, exactly one Project `applyIntent`/reducer call mutates that candidate and
independently consumes the fresh apply-phase `{F}` capability.

Canvas must not:

- receive the Project candidate;
- run a preview Project reducer;
- construct a second candidate;
- execute F13;
- own a reset witness or permit;
- mutate a Canvas Y.Doc as part of Project route reset;
- reinterpret the twelve current-route fields.

The temporary Y.Doc used to verify G is validation material only and must be
destroyed before Project post-state validation. It is never the new live Canvas
replica; installation of the staged Canvas shard remains Project/native composition
work after the Project route frame crosses its durable barrier.

### Canonical carrier and conflict behavior

For one G, the complete carrier section subjects, section order and section bytes
are canonical. Same G with unequal complete carrier bytes is a dependency-subject
conflict only when both candidates independently pass all structural, hash,
authority and Canvas semantic validation.

A malformed, over-cap, bad-signature, wrong-subject or semantically invalid second
candidate rejects independently and must not quarantine an already valid carrier.
This prevents malformed bytes from becoming a subject-conflict denial of service.

Two independently valid unequal candidates for one G are retained as separate
candidates and make the subject permanently conflicted for that Project epoch.
Arrival order, prior ACK, GC and local-store checkpointing must never select one as
winner or erase the conflict high-water.

### Canvas falsifiers

Revision 5 Canvas conformance is falsified if:
...
- Canvas reads C or current Project route facts;
- G identity F is treated as the untrusted lookup hint;
- F lookup occurs before C-hint/G-identity equality;
- Project candidate mutation occurs before final F13;
- a Canvas verifier uses React Flow or another ambient store;
- malformed second bytes quarantine a valid first carrier;
- GC or checkpointing converts a conflicted G back into a unique carrier;
