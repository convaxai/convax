...
`CanvasRouteActivationV2` is declared exactly once by the terminal activation
contract in section 18.7. This earlier route union does not define a reduced shape.
...
## 9. Native frame commit and deterministic reopen## 9. Native frame commit and deterministic reopen
...
exact manifest, declared charge and stage. Process-only attempts and connection ids
never enter this adapter or any durable record:
...
interface RemoteIngressReservationRecordV2 {
  ...
  exactManifestCoreJcs: Uint8Array
  declaredByteLength: Uint64V2
  stage: RemoteIngressStageV2
}
```

The stable index is keyed by the stable transfer identity and exact manifest. Kernel
keeps the authenticated attempt in its private live registry and may reconnect only
that same manifest/reservation. Every durable transition returns a private live
receipt bound by object identity and exact durable record digest:
...
`stagedGenesisCheckpointObjectDigest` spelling rejects. F is obtained only from G's
canonical Canvas identity inside F13, avoiding a second current-route authority or
hash cycle.

## 19. Revision-5 Project reset portable contract

This section owns the final ProjectIndex reset dependencies, portable DTOs and durable reset manifest. It has no legacy alias or decoder.
...
## 20. Revision-5 Project dependency store, reset gate and sole apply

This terminal section replaces every earlier conflicting dependency-cache record,
fold, reservation, ACK, checkpoint, GC, reset witness and candidate-apply statement.
Project/node is the sole native writer. No directory scan, renderer store, Peer
connection or old Revision decoder is a parallel authority.

### Four immutable local record classes

There are exactly four authoritative dependency-cache JCS record classes: wanted
root, carrier index, cache transaction and cache head. Checkpoint pages are cache
transactions, not a fifth record class.

Within the Project-owned record declarations below,
`ControlOwnedResetAuthorizationCarrierSectionV2` denotes only a type-only reference
to the manifest-selected Control artifact's exact
`DocumentShardResetAuthorizationCarrierSectionV2`. It is not a Project declaration,
structural alias, decoder or compatibility surface.

```ts
import type {
  CanvasGenesisProofCarrierSectionV2 as CanvasOwnedGenesisProofCarrierSectionV2,
} from "@convax/canvas/collaboration"

type OrdinarySha256V2 = import("control-plane").OrdinarySha256V2

type DependencyCarrierSubjectV2 =
  | Readonly<{
      kind: "canvas-genesis-proof-carrier"
      scope: DocumentScopeV2 & {
        docKind: "canvas"
        docId: CanvasIdV2
      }
  ...
  | Readonly<{
      kind: "document-shard-reset-authorization-carrier"
      scope: ProjectIndexScopeV2
      subjectDigest: DigestV2
    }>

type DependencyWantedOwnerV2 =
  | { readonly kind: "pending-causal-frame"; readonly ownerRecordDigest: DigestV2 }
  | {
      readonly kind: "staged-operation"
      readonly actorId: ActorIdV2
      readonly operationId: Id128V2
      readonly intentDigest: DigestV2
    }
  | { readonly kind: "retained-recovery"; readonly ownerRecordDigest: DigestV2 }

interface DependencyWantedRootRecordV2 {
  readonly format: "convax.dependency-wanted-root-record/2"
  readonly projectId: ProjectIdV2
  readonly projectEpoch: Id128V2
  readonly subject: DependencyCarrierSubjectV2
  readonly owner: DependencyWantedOwnerV2
}
```

Wanted roots have no state, status, attempt, connection, clock or mutable field.

```ts
type DependencyCarrierIndexRecordV2 =
  | Readonly<{
      format: "convax.dependency-carrier-index-record/2"
      projectId: ProjectIdV2
      projectEpoch: Id128V2
      subject: Extract<
        DependencyCarrierSubjectV2,
        Readonly<{ kind: "canvas-genesis-proof-carrier" }>
      >
      carrierFormat: "CVXCGP02"
      ordinarySha256: OrdinarySha256V2
      exactByteLength: Uint64V2
      sections: readonly CanvasOwnedGenesisProofCarrierSectionV2[]
    }>
  | Readonly<{
      format: "convax.dependency-carrier-index-record/2"
      projectId: ProjectIdV2
      ...
      >
      carrierFormat: "CVXRSA02"
      ordinarySha256: OrdinarySha256V2
      exactByteLength: Uint64V2
      sections: readonly ControlOwnedResetAuthorizationCarrierSectionV2[]
    }>
```

The Project store imports the Canvas and Control closed section classifiers. It does
not redeclare either carrier ABI. The index records no winner, quarantine, wanted
root, source member, native path or connection state.

### Fold, quota and transitions

```ts
interface DependencyQuotaDeltaV2 {
  readonly projectObjectsAdd: Uint64V2
  readonly projectObjectsRemove: Uint64V2
  readonly projectBytesAdd: Uint64V2
  readonly projectBytesRemove: Uint64V2
  readonly memberObjectsAdd: Uint64V2
  readonly memberObjectsRemove: Uint64V2
  readonly memberBytesAdd: Uint64V2
  readonly memberBytesRemove: Uint64V2
}

interface ZeroDependencyQuotaDeltaV2 {
  readonly projectObjectsAdd: "0"
  readonly projectObjectsRemove: "0"
  readonly projectBytesAdd: "0"
  readonly projectBytesRemove: "0"
  readonly memberObjectsAdd: "0"
  readonly memberObjectsRemove: "0"
  readonly memberBytesAdd: "0"
  readonly memberBytesRemove: "0"
}

interface DependencyCheckpointEntryCountsV2 {
  readonly activeReservationCount: Uint32V2
  readonly liveCandidateCount: Uint32V2
  readonly candidateRetentionRootCount: Uint32V2
  readonly manifestEquivocationHighWaterCount: Uint32V2
  readonly subjectConflictHighWaterCount: Uint32V2
  readonly memberQuotaCount: Uint32V2
  readonly projectQuotaCount: "1"
  readonly totalEntryCount: Uint32V2
}

interface DependencyActiveReservationV2 {
  ...
  readonly projectObjectCharge: "1"
  readonly projectByteCharge: Uint64V2
  readonly memberObjectCharge: "1"
  readonly memberByteCharge: Uint64V2
}

type DependencyRetentionRootV2 =
  | { readonly kind: "pending-causal-frame"; readonly rootDigest: DigestV2 }
  | { readonly kind: "staged-operation"; readonly rootDigest: DigestV2 }
  | { readonly kind: "retained-recovery"; readonly rootDigest: DigestV2 }
  | { readonly kind: "accepted-causal-frame"; readonly rootDigest: DigestV2 }
  | { readonly kind: "checkpoint"; readonly rootDigest: DigestV2 }
  | { readonly kind: "staged-route"; readonly rootDigest: DigestV2 }
  | { readonly kind: "live-route"; readonly rootDigest: DigestV2 }
  | { readonly kind: "reset-history"; readonly rootDigest: DigestV2 }
  | { readonly kind: "quarantine"; readonly rootDigest: DigestV2 }

interface DependencyLiveCandidateV2 {
  readonly subject: DependencyCarrierSubjectV2
  readonly carrierIndexRecordDigest: DigestV2
  readonly ordinarySha256: DigestV2
  readonly exactByteLength: Uint64V2
  readonly chargeOwnerSourceMemberId: MemberIdV2
  readonly disposition: "eligible" | "quarantined" | "gc-marked"
}
```

The closed `DependencyCacheTransitionV2` union is:

```ts
type DependencyCacheTransitionV2 =
  | {
      readonly kind: "reserve"
      readonly reservation: DependencyActiveReservationV2
      readonly quotaDelta: DependencyQuotaDeltaV2
    }
  | {
      readonly kind: "transfer-manifest-equivocation"
      readonly stableKey: StableRemoteTransferKeyV2
      readonly sourceMemberId: MemberIdV2
      readonly firstManifestDigest: DigestV2
      readonly secondManifestDigest: DigestV2
      readonly firstSignedOfferEvidenceDigest: DigestV2
      readonly secondSignedOfferEvidenceDigest: DigestV2
      readonly quotaDelta: ZeroDependencyQuotaDeltaV2
    }
  | {
      readonly kind: "install-exact"
      readonly reservationTransactionRecordDigest: DigestV2
      readonly wantedRootRecordDigest: DigestV2
      readonly carrierIndexRecordDigest: DigestV2
      readonly installMode: "new-bytes" | "exact-duplicate"
      readonly quotaDelta: DependencyQuotaDeltaV2
    }
  | {
      readonly kind: "dependency-subject-conflict"
      readonly subject: DependencyCarrierSubjectV2
      readonly firstCarrierIndexRecordDigest: DigestV2
      readonly secondCarrierIndexRecordDigest: DigestV2
      readonly firstOrdinarySha256: DigestV2
      readonly secondOrdinarySha256: DigestV2
      readonly firstExactByteLength: Uint64V2
      readonly secondExactByteLength: Uint64V2
      readonly triggeringReservationTransactionRecordDigest: DigestV2
      readonly quotaDelta: ZeroDependencyQuotaDeltaV2
    }
  | {
      ...
      readonly reservationTransactionRecordDigest: DigestV2
      readonly sourceMemberId: MemberIdV2
      readonly releasedOrdinarySha256: DigestV2
      readonly releasedExactByteLength: Uint64V2
      readonly reason:
  ...
  | {
      ...
      readonly carrierIndexRecordDigest: DigestV2
      readonly fromRoot: DependencyRetentionRootV2 | null
      readonly toRoot: DependencyRetentionRootV2 | null
      readonly cancelledGcMarkTransactionRecordDigest: DigestV2 | null
      readonly quotaDelta: ZeroDependencyQuotaDeltaV2
    }
  | {
      readonly kind: "gc-mark"
      readonly carrierIndexRecordDigest: DigestV2
      readonly firstScanHeadRecordDigest: DigestV2
      readonly secondScanHeadRecordDigest: DigestV2
      readonly quotaDelta: ZeroDependencyQuotaDeltaV2
    }
  | {
      readonly kind: "gc-release"
      readonly gcMarkTransactionRecordDigest: DigestV2
      readonly carrierIndexRecordDigest: DigestV2
      readonly chargeOwnerSourceMemberId: MemberIdV2
      readonly deletedCarrierOrdinarySha256: DigestV2
      readonly deletedExactByteLength: Uint64V2
      readonly deletedConstituentObjectDigests: readonly DigestV2[]
      readonly quotaDelta: DependencyQuotaDeltaV2
    }
  | {
      ...
      readonly priorPageTransactionRecordDigest: DigestV2 | null
      readonly entries: readonly DependencyCheckpointEntryV2[]
      readonly quotaDelta: ZeroDependencyQuotaDeltaV2
    }
  | {
      readonly kind: "checkpoint"
      readonly checkpointId: Id128V2
      readonly orderedPageTransactionRecordDigests: readonly DigestV2[]
      readonly entryCounts: DependencyCheckpointEntryCountsV2
      readonly quotaDelta: ZeroDependencyQuotaDeltaV2
    }
```

Every transition carries an exact quota delta; there is no default.

The validator derives the delta from the transition and requires byte equality with
the carried value. Callers cannot choose accounting:

```ts
const ZERO: ZeroDependencyQuotaDeltaV2 = {
  projectObjectsAdd: "0",
  projectObjectsRemove: "0",
  projectBytesAdd: "0",
  projectBytesRemove: "0",
  memberObjectsAdd: "0",
  memberObjectsRemove: "0",
  memberBytesAdd: "0",
  memberBytesRemove: "0",
}

function reserveDelta(B: Uint64V2): DependencyQuotaDeltaV2 {
  return {
    projectObjectsAdd: "1",
    projectObjectsRemove: "0",
    projectBytesAdd: B,
...
function releaseDelta(B: Uint64V2): DependencyQuotaDeltaV2 {
  return {
    projectObjectsAdd: "0",
    projectObjectsRemove: "1",
    projectBytesAdd: "0",
    projectBytesRemove: B,
    memberObjectsAdd: "0",
    memberObjectsRemove: "1",
    memberBytesAdd: "0",
    memberBytesRemove: B,
  }
}
...
| Transition | Required exact `quotaDelta` |
| --- | --- |
| `reserve` | `reserveDelta(reservation.exactByteLength)` |
| `transfer-manifest-equivocation` | `ZERO` |
| `install-exact/new-bytes` | `ZERO` |
| `install-exact/exact-duplicate` | `releaseDelta(original reservation exactByteLength)` |
| `dependency-subject-conflict` | `ZERO` |
| `release-reservation` | `releaseDelta(releasedExactByteLength)` |
| `reclassify` | `ZERO` |
| `gc-mark` | `ZERO` |
| `gc-release` | `releaseDelta(deletedExactByteLength)` |
| `checkpoint-page` | `ZERO` |
| `checkpoint` | `ZERO` |

The reservation source member equals its stable-key source member; both object
charges equal `"1"`; both byte charges equal `exactByteLength`. Install resolves one
live reserve and requires wanted, subject, ordinary SHA and length equality with its
index. Release source member, SHA and length equal the referenced live reserve and
is invalid after install, conflict, prior release or any root. GC release charge
owner, SHA and length equal the installed candidate and index.

For a nonzero member delta, `affectedSourceMemberId` is the exact charge owner and
both resulting member totals are non-null. For `ZERO`, all three are null. Head
totals subtract before adding, reject underflow or Uint64 overflow and check all
four caps before CAS. Checkpoint Project/member quota entries equal the prior folded
totals and cannot reset or normalize them.

```ts
interface DependencyCacheTransactionRecordV2 {
  readonly format: "convax.dependency-cache-transaction-record/2"
  readonly projectId: ProjectIdV2
  readonly projectEpoch: Id128V2
  readonly expectedPriorHeadRecordDigest: DigestV2 | null
  readonly priorTransactionRecordDigest: DigestV2 | null
  readonly transition: DependencyCacheTransitionV2
}

interface DependencyCacheHeadRecordV2 {
  readonly format: "convax.dependency-cache-head-record/2"
  readonly projectId: ProjectIdV2
  readonly projectEpoch: Id128V2
  readonly generation: Uint64V2
  readonly priorHeadRecordDigest: DigestV2 | null
  readonly transactionRecordDigest: DigestV2
  readonly transitionKind: DependencyCacheTransitionV2["kind"]
  readonly historyFloorTransactionRecordDigest: DigestV2 | null
  readonly resultingProjectObjects: Uint64V2
  readonly resultingProjectBytes: Uint64V2
  readonly affectedSourceMemberId: MemberIdV2 | null
  readonly resultingMemberObjects: Uint64V2 | null
  readonly resultingMemberBytes: Uint64V2 | null
}
```
...
```text
new.generation == prior.generation + 1
new.priorHeadRecordDigest == digest(prior)
tx.expectedPriorHeadRecordDigest == digest(prior)
new.transactionRecordDigest == digest(tx)
new.transitionKind == tx.transition.kind
new quotas == subtract(prior, delta.remove) + delta.add
```

The fold subtracts first and rejects underflow, then adds and rejects overflow. It
checks the Project and source-member object and byte dependency-ingress caps.

### Permanent high-water and paged checkpoint

```ts
type DependencyCheckpointEntryV2 =
  | {
      readonly kind: "active-reservation"
      readonly reservationTransactionRecordDigest: DigestV2
      readonly reservation: DependencyActiveReservationV2
    }
  | {
      readonly kind: "live-candidate"
      readonly candidate: DependencyLiveCandidateV2
    }
  | {
      readonly kind: "candidate-retention-root"
      readonly carrierIndexRecordDigest: DigestV2
      readonly retentionRoot: DependencyRetentionRootV2
    }
  | {
      readonly kind: "manifest-equivocation-high-water"
      readonly stableKey: StableRemoteTransferKeyV2
      readonly sourceMemberId: MemberIdV2
      readonly firstManifestDigest: DigestV2
      readonly secondManifestDigest: DigestV2
  ...
  | {
      readonly kind: "subject-conflict-high-water"
      readonly subject: DependencyCarrierSubjectV2
      readonly firstCarrierIndexRecordDigest: DigestV2
      readonly secondCarrierIndexRecordDigest: DigestV2
      readonly firstOrdinarySha256: DigestV2
  ...
  | {
      readonly kind: "member-quota"
      readonly sourceMemberId: MemberIdV2
      readonly retainedObjects: Uint64V2
      readonly retainedBytes: Uint64V2
    }
  | {
      readonly kind: "project-quota"
      readonly retainedObjects: Uint64V2
      readonly retainedBytes: Uint64V2
    }
```

Manifest-equivocation and subject-conflict high-water are permanently monotonic in
one Project epoch. Release, GC, checkpoint compaction and reconnect cannot delete
them. A checkpoint carries them completely. A subject conflict permanently retains
both candidates' index identity, SHA and length. Conflict raw bytes, index and
sections remain charged to their original source members and Project dependency
aggregate quota; they never move to the generic 128 MiB quarantine cap. Only an
explicit destructive Project-epoch reset may clear the old epoch store.

The closed metadata limits are:

```ts
const DEPENDENCY_LOCAL_RECORD_MAX_JCS_BYTES = 65_536
const DEPENDENCY_CHECKPOINT_PAGE_ENTRIES_MAX = 128
const DEPENDENCY_CHECKPOINT_PAGES_MAX = 256
const DEPENDENCY_TRANSITIONS_AFTER_FLOOR_MAX = 8_192
const DEPENDENCY_METADATA_RECORDS_PER_PROJECT_EPOCH_MAX = 32_768
const DEPENDENCY_CARRIER_SECTIONS_MAX = 256
const DEPENDENCY_SIGNED_OFFER_EVIDENCE_PER_STABLE_KEY_MAX = 2
```

All 4352 member-quota entries must be paged. Increasing one record beyond 64 KiB,
truncating members or using directory enumeration as a completeness fallback is
forbidden. A `checkpoint-page` is a prewritten immutable transaction object and
never enters the head. One final checkpoint transaction/head publishes all pages.
Before publishing, the sole writer regenerates the checkpoint from the prior folded
head and compares every entry. Page directory enumeration is not evidence of
completeness.

Retention roots are normalized as independent `candidate-retention-root` entries;
they never form an unbounded nested array on a live-candidate entry. Their unique
key is exact
`{carrierIndexRecordDigest,retentionRoot.kind,retentionRoot.rootDigest}`. Every root
must reference exactly one live-candidate in the same checkpoint. Dangling and
duplicate roots reject. All checkpoint entries use one closed kind-specific sort
key encoded as restricted-JCS UTF-8 bytes and are strictly increasing before pages
are cut as consecutive slices. A page has 1..128 entries, its complete transaction
record remains at most 65,536 bytes, there are at most 256 pages and the complete
checkpoint remains within the 32,768-entry metadata bound.

Checkpoint counts and page linkage obey:

```text
totalEntryCount
  == activeReservationCount
   + liveCandidateCount
   + candidateRetentionRootCount
   + manifestEquivocationHighWaterCount
   + subjectConflictHighWaterCount
   + memberQuotaCount
   + 1

orderedPageTransactionRecordDigests.length == pageCount
page ordinals == 0..pageCount-1
each page contains 1..128 entries
sum(page.entries.length) == totalEntryCount
all pages and the terminal checkpoint share expectedPriorHeadRecordDigest
all pages and the terminal checkpoint share checkpointId
page 0 priorPageTransactionRecordDigest == null
every later page points to the exact prior page transaction digest
terminal checkpoint priorTransactionRecordDigest == last page digest
terminal head historyFloorTransactionRecordDigest == terminal checkpoint transaction digest
later ordinary heads inherit that exact history floor until the next complete checkpoint
```

### Stable transfer and live reservation fold

For the same stable key and manifest digest, reconnect reuses the original reserve
transaction, wanted, subject, writer, staging and quota only when wanted, subject,
SHA, length, source member and charge are all equal. It creates no second reserve
record. Any different binding is not reconnect.

For the same stable key and a different manifest digest, the sole writer persists
permanent transfer-manifest-equivocation high-water before a second payload
allocation. Both authenticated signed offers enter bounded evidence, the second
payload charge is zero, and the original legitimate reservation and attempt remain
unchanged. That stable key can mint no later receipt or ACK.

The current fold for `{kind,scope,subjectDigest}` includes every eligible,
quarantined and gc-marked candidate, every live reservation, and permanent
subject-conflict high-water. If another live reservation declares unequal
`{ordinarySha256,exactByteLength}`, install and ACK wait until it either completes
as a semantic-valid conflict candidate or legally releases. Arrival order cannot
ACK the first candidate early.

A legal release proves all of the following before its one exact negative quota
delta:

```text
partial staging deleted and staging directory fsynced
AND no raw carrier object
AND no carrier index
AND no install transaction
AND no quarantine or conflict high-water
AND no retention root
AND reservation not previously released
```

Repeated release is corruption, never an idempotent second debit.

### Install, malformed carrier, conflict and ACK

The exact order is:

```text
authenticated wanted owner
-> wanted record fsync
-> sole-writer reserve tx/head CAS and fsync
-> quota-bound staging allocation
-> chunk/staging fsync
-> full length and ordinary-SHA validation
-> complete carrier and section structural validation
-> complete semantic validation
-> raw carrier and constituent immutable objects fsync
-> carrier index fsync
-> sole-writer current fold including same-subject live reservations
-> install-exact or dependency-subject-conflict tx/head CAS and fsync
-> same sole-writer frame current-fold ACK gate
-> mint private receipt and outbound enqueue
```

A malformed, structurally invalid or semantically invalid second candidate does not
create subject conflict, publish raw carrier/index or quarantine the valid first
candidate. Its partial staging is deleted and fsynced before legal reservation
release. Only two complete semantic-valid candidates for the same subject with
unequal ordinary bytes commit `dependency-subject-conflict`.

After conflict, both raw/index/section closures remain charged and retained. A prior
ACK is historical proof of durability at its enqueue point and is not revoked, but
no old candidate remains a winner. Every affected accepted shard enters terminal
recovery-required: edit, incoming admission, dependency/replica ACK, checkpoint,
prune and GC are closed. If affected shards cannot be proven, the entire Project
collaboration closes. Recovery requires protected external adjudication or explicit
shard/Project reset.

```ts
type RemoteDependencyCarrierRefV2 =
  import("control-plane").RemoteDependencyCarrierRefV2
...
This is a Project/node module-private live-set registered affine capability with no
codec, serializer, clone, persistence, browser-safe export or public constructor.
Only Project owns its brand, registry, mint, current-fold validation and consume.
Desktop can compose the Project gate with the Control outbound adapter but cannot
construct or inspect the receipt. In the sole-writer queue the gate reads the exact
current head, folds it, proves wanted, install, unique eligible candidate, no
unequal reservation, no high-water and complete bytes, then mints the receipt and
synchronously enqueues the ACK bound to that head generation. From head read through
mint and enqueue there is no await, callback, IPC or queue release. Crash-time ACK
replay enters the queue again and passes the current fold; historical receipts are
not replay tokens. An exact duplicate still writes a new install transaction/head
for the current wanted and releases the reservation quota exactly once.

### Native identity and GC

All four record classes use the existing identity:

```text
localStoreRecordDigest = SHA-256(
  "convax.local-project-store-record-digest/2\0"
  || UTF8(record.format)
  || "\0"
  || JCS(record)
)

record key = nativeObjectKey(
  record-object-kind,
  decoded localStoreRecordDigest
)

raw carrier key = nativeObjectKey(
  "dependency-carrier-bytes",
  ordinarySha256
)
```

Length is not part of the raw key. Every read verifies actual SHA and actual length;
same SHA with another length is store-corruption/hash-collision quarantine. Subject
refs are rebuildable non-authoritative projections. Candidate completeness comes
only from the folded head, never from directory scanning.

GC performs a complete first reverse-root scan. The second scan runs in the
sole-writer queue against the latest head and proves that the subject still has no
root, reservation or conflict. `gc-mark` releases no quota. A new root cancels the
mark in the same head CAS with `reclassify.cancelledGcMarkTransactionRecordDigest`
pointing at that mark. The old deleter then fails its current-fold/CAS check and
cannot delete.

Delete order is:

```text
sole-writer verifies current fold remains gc-marked and rootless
-> delete raw carrier
-> delete exclusively owned constituent bytes
-> fsync every affected directory
-> commit gc-release tx/head
-> apply exact negative quota delta once
```

Recovery continues deletion when raw bytes and mark both exist. If raw bytes are
missing with a mark, it verifies all scheduled objects are absent and completes
`gc-release`. Raw missing without a mark is store corruption and read-only
quarantine. A cancelled mark can never authorize deletion. Conflict high-water and
candidate metadata never enter GC within a Project epoch. Transaction/index
metadata is cleaned only below a completely published paged-checkpoint floor.

### Reset witness, F13, permit and sole apply

Project imports the Control-owned current-authority input exactly; it does not
redeclare or weaken `ExactJcsInputBytesV2`:

```ts
type DocumentShardResetCurrentAuthorityBytesV2 =
  import("control-plane").DocumentShardResetCurrentAuthorityBytesV2

type VerifyDocumentShardResetAuthorityInputV2 =
  import("control-plane").VerifyDocumentShardResetAuthorityInputV2
```

The exact definition exists only in the Control artifact. These values are fresh,
defensive-copied, required process input,
never an ambient lookup. Claimed/history sections live in C; current authority comes
from the current accepted authority head through a Project-private witness lease.

All twelve current-route fields are consumed, and reset requires:

```text
currentStagedResetAuthorizationCarrierDigest
  == resetCommit.resetAuthorizationCarrierDigest
  == C
```

The only local reset order is:

```text
1. Outside the queue, parse exact C; validate nested G; prove C hint F equals
   authenticated G identity F.
2. Enter the exclusive document queue.
3. Reconstruct the latest accepted base; Kernel freezes exact base set {F}.
4. Clone one isolated unmutated candidate from that base.
5. The discover phase consumes exactly F.
6. Acquire the Project-private current-authority witness; A1.
7. Export fresh currentAuthority plus fresh current route facts.
8. Run initial F13; its phase consumes exactly F.
9. A2; issue the private affine reset permit.
10. Atomically move the permit from issued to consuming.
11. A3; export another fresh currentAuthority, current head and all 12 route facts.
12. Run final F13 against the same still-unmutated candidate; consume exactly F.
13. A4.
14. Consume the permit and synchronously return the sole reducer-entry capability.
15. In the same call frame, with no await/callback/IPC, enter the sole reducer.
16. The apply phase independently consumes exact F and applies one typed reset intent.
17. Move permit state to consumed; every repeated entry rejects.
18. Require discover/initial/final/apply ledgers all equal exactly {F} and the same bytes.
19. Validate post state and evidence.
20. Cross the object, outbox-or-inbox, journal and head durability barrier.
```

The candidate has no mutation before final F13. F13 never enters the reducer; the
reducer never reruns F13; apply occurs once.

Incoming/recovery uses:

```text
resolve exact C/G/F
-> clone one unmutated candidate
-> fresh witness/currentAuthority/current route
-> one complete reset-f13-incoming
-> one-shot admission/permit consumption
-> same-frame sole reducer and apply once
-> post/evidence/durability
```

There is no second reset verifier, ambient WeakMap fact, old route DTO alias or
document-wide version guard.

### Project falsifiers

Revision 5 Project conformance is falsified if:

- an unequal live reservation permits another candidate to install or ACK;
- a malformed second carrier creates subject-conflict high-water;
- another transition can enter between receipt mint and ACK enqueue;
- checkpointing erases manifest or subject high-water;
- conflict bytes stop charging their original source/Project dependency quota;
- an old deleter removes bytes after a new root cancels `gc-mark`;
- raw-missing plus marked recovery neither releases quota nor closes the store;
- 4352 member quotas require a directory scan or a record over 64 KiB;
- the candidate mutates before final F13;
- one reset enters the reducer or apply twice;
- any `*Bytes: Uint8Array` or `liveScopeManifest` current-authority alias decodes;
- an active authority is selected by directory discovery instead of the stable
  reviewed pointer.
