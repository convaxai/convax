...
Status: **Revision 4 normative merge candidate** for the v10 canonical specification. This
appendix is the control-plane artifact for membership, session, rendezvous, Peer transport,
checkpoint certification, causal-floor certification, registry and cutoff wire
semantics. It does not define ProjectIndex or Canvas records, reducers, intents or
projection rules.

The words MUST, MUST NOT, SHOULD and MAY are normative. All object schemas are
closed. Unknown keys, missing required keys, noncanonical values and trailing bytes
fail closed before a state transition.
