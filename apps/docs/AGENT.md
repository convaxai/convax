...
| UI from registry             | `bunx @cloudflare/nimbus-docs add <slug>`. Register in `src/components.ts` if used in MDX.                   |
| Feature recipe               | `bunx @cloudflare/nimbus-docs add <feature-slug>`. Pipe the printed brief to your agent.                     |
| Custom page route            | Add a file under `src/pages/`.                                                                               |
| Custom OG style              | Edit `src/pages/og/_og-card-config.ts`.                                                                      |
| Check for updates            | `bunx @cloudflare/nimbus-docs outdated` — starter files behind their tag + registry components behind.       |
| Upgrade a starter file       | `bunx @cloudflare/nimbus-docs diff <file>` to review, `diff --apply <file>` to pull a clean upstream change. |
| Upgrade a registry component | `bunx @cloudflare/nimbus-docs add <slug> --overwrite`, then review with `git diff`.                          |

List installable items: `bunx @cloudflare/nimbus-docs list`.
