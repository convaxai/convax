...
/** Internal acceleration with exact portable parity and fail-safe fallback. */
export async function nativeOrdinarySha256(
  bytes: Uint8Array,
  ...
  if (subtle) {
    try {
      ...
      // Native hashing is acceleration-only; portable SHA-256 remains authoritative.
    }
  }
...
export async function nativeCanonicalStateDigest(
  ownerSchemaDigest: Digest | string,
  exactCanonicalState: Uint8Array,
  subtle: Pick<SubtleCrypto, "digest"> | null | undefined = globalThis.crypto?.subtle,
): Promise<Digest> {
  if (!(exactCanonicalState instanceof Uint8Array)) throw new TypeError("Canonical state must be Uint8Array")
  if (subtle) {
    try {
      ...
      preimage[domain.byteLength + 1 + schema.byteLength] = 0
      preimage.set(exactCanonicalState, domain.byteLength + 2 + schema.byteLength)
      const digest = new Uint8Array(await subtle.digest("SHA-256", preimage))
      if (digest.byteLength === 32) return bytesToHex(digest)
    } catch {
      // Native hashing is acceleration-only; the portable implementation is authoritative.
    }
  }
  return canonicalStateDigest(ownerSchemaDigest, exactCanonicalState)
}
...
export function bytesToHex(bytes: Uint8Array): Digest {
  if (!(bytes instanceof Uint8Array) || bytes.byteLength !== 32)
    throw new TypeError("Digest bytes must contain exactly 32 bytes")
...
function sha256Bytes(input: Uint8Array): Uint8Array {
  ...
  for (let block = 0; block < blockCount; block += 1) {
    ...
    for (let word = 16; word < 64; word += 1) {
      const s0 =
        rotateRight(schedule[word - 15]!, 7) ^ rotateRight(schedule[word - 15]!, 18) ^ (schedule[word - 15]! >>> 3)
      const s1 =
        rotateRight(schedule[word - 2]!, 17) ^ rotateRight(schedule[word - 2]!, 19) ^ (schedule[word - 2]! >>> 10)
