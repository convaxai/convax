...
export function createAgentCanvasInstructions(input: {
  ...
  const instructions: string[] = [
    "Canvas edges are directed from source to target: source is the card's right-side output and target is the card's left-side input. For a given card, only incomingNodeIds (edges whose target is that card) are inputs; outgoingNodeIds are outputs and must not be treated as inputs.",
  ]
