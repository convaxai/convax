...
describe("NodeProjectTeamMemberIdentityStore", () => {
  test("publishes one stable per-Project identity and reopens canonical bytes", async () => {
    ...
    const store = new NodeProjectTeamMemberIdentityStore(path.join(root, "ids"), () => memberId)
    const projectId = parseProjectId("project-a")
    expect(await store.resolve(projectId)).toBe(memberId)
    expect(await new NodeProjectTeamMemberIdentityStore(path.join(root, "ids")).resolve(projectId)).toBe(memberId)
