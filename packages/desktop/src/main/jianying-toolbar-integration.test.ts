...
const isIsolatedRun = process.env.CONVAX_JIANYING_TOOLBAR_TEST === "isolated"
...
async function runIsolatedTestFile() {
  const child = Bun.spawn({
    ...
    env: { ...globalThis.process.env, CONVAX_JIANYING_TOOLBAR_TEST: "isolated" },
    stderr: "pipe",
    stdout: "pipe",
  })
  ...
  if (exitCode !== 0) throw new Error(`Isolated JianYing toolbar tests failed:\n${stdout}\n${stderr}`)
}

if (!isIsolatedRun) {
  test("runs the Electron-mocked JianYing toolbar integration in isolation", runIsolatedTestFile)
}
