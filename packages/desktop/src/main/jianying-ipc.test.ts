...
const isIsolatedRun = process.env.CONVAX_JIANYING_IPC_TEST === "isolated"
...
async function runIsolatedTestFile() {
  const child = Bun.spawn({
    ...
    env: { ...globalThis.process.env, CONVAX_JIANYING_IPC_TEST: "isolated" },
    stderr: "pipe",
    stdout: "pipe",
  })
  ...
  if (exitCode !== 0) throw new Error(`Isolated JianYing IPC tests failed:\n${stdout}\n${stderr}`)
}

if (!isIsolatedRun) {
  test("runs the Electron-mocked JianYing IPC contract in isolation", runIsolatedTestFile)
} else {
