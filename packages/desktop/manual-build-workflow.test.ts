...
describe("manual Desktop release workflow", () => {
  ...
  test("publishes a complete generic feed only after checksums and signatures pass", () => {
    ...
    expect(workflow).toContain("Publish or verify immutable GitHub Release")
    expect(workflow).toContain("asset $asset differs; version reuse is forbidden")
    expect(workflow).toContain('cmp -s "release-assets/$asset" "$existing_assets/$asset"')
