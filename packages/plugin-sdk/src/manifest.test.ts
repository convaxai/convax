...
describe("complete convax.plugin/8 portable ABI", () => {
  ...
  test("keeps Host API authoring and runtime negotiation modes distinct", () => {
    ...
    expect(() =>
      parsePortablePluginManifestV8(candidate, { hostApiMode: "authoring" }),
    ).toThrow("unknown Plugin API")
    expect(() =>
      parsePortablePluginManifestV8(
        webManifest({
          hostApi: { major: 1, optional: ["Future.timeline.inspect"], required: ["host.context.get"] },
        }),
      ),
  ...
  test("rejects every retired manifest schema and unknown portable fields", () => {
    for (let version = 1; version <= 7; version += 1) {
      expect(() =>
        parsePortablePluginManifestV8({ ...webManifest(), schema: `convax.plugin/${version}` }),
      ).toThrow("must use convax.plugin/8")
    }
    expect(() => parsePortablePluginManifestV8({ ...webManifest(), hidden: true })).toThrow(
      "unsupported field: hidden",
    )
    expect(() =>
      parsePortablePluginManifestV8({
        ...webManifest(),
  ...
  test("enforces runtime, generation, Agent, Skill, and selection-action references", () => {
    ...
    expect(
      parsePortablePluginManifestV8(capabilityOnly).contributes.capabilities
        ?.exports[0]?.operation,
    ).toBe("timeline.inspect")
    expect(() =>
      parsePortablePluginManifestV8({ ...capabilityOnly, runtime: undefined }),
    ).toThrow("capability exports require a verified mcp-stdio runtime")
    expect(() =>
      parsePortablePluginManifestV8({
        ...toolManifest(),
        contributes: {
