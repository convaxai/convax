...
import {
  ...
  queryCanvasTopLevelObstacleCollisions,
...
function placeCreatedNodes(
  ...
  const localObstacles: { x: number; y: number; width: number; height: number }[] = []
  ...
  for (const spec of [...specs].sort((a, b) => uint32ToNumber(a.ordinal) - uint32ToNumber(b.ordinal))) {
    ...
    while (true) {
      ...
      const collisions = [
        ...queryCanvasTopLevelObstacleCollisions(base, position, spec.size, 24).map((obstacle) => ({
          ...obstacle.position,
          ...obstacle.size,
        })),
        ...localObstacles.filter((obstacle) =>
      ...
      for (const obstacle of collisions) {
        if (
          ...
          nextX = Math.max(nextX, obstacle.x + obstacle.width + 24)
        }
      }
      if (nextX === position.x) break
      position = { x: nextX, y: position.y }
      if (++iterations > base.nodes.size + localObstacles.length + 1 || !Number.isFinite(position.x) || position.x > 10_000_000)
        throw new CanvasSchemaError("placement-unavailable", "Causal placement cannot find a bounded position")
    }
    const obstacle = { ...position, ...spec.size }
