...
const member = (byte: number) => parseMemberIdV2(id(byte))
const key = (byte: number) => parsePublicKeyV2(encodeBase64urlV2(new Uint8Array(32).fill(byte)))
...
describe("R5 membership codecs", () => {
  test("closes reservation and member-add proof over exact R5 fields", () => {
    const reservationCore = {
      ...
      expectedMembershipSequence: parseUint64V2("2"),
    ...
    const proofCore = {
      ...
      targetMemberMutationCounter: parseUint64V2("1"),
      serverNonce: id(9),
  ...
  test("rejects digest tampering, unknown fields, and signature-branch substitution", () => {
    const core = {
      format: "convax.mutation-proof-core/2" as const,
      mutationId: id(1), challengeDigest: digest("1"), projectId: parseProjectIdV2("project-a"), projectEpoch: id(2), membershipEpoch: id(3), expectedMembershipSequence: parseUint64V2("1"), requesterMemberId: member(4), targetMemberId: member(5), targetMemberMutationCounter: parseUint64V2("1"), serverNonce: id(6), purpose: "member-add" as const, targetMemberSigningPublicKey: key(5), initialRole: "viewer" as const, adminCapabilityDigest: digest("2"),
    }
