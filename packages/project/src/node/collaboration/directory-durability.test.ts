...
import {
  NodeDirectoryDurabilityError,
  NodeDirectoryDurabilityUnavailableError,
  fsyncProjectDirectory,
  isNodeDirectoryDurabilityUnavailable,
...
describe("Project/node directory durability boundary", () => {
  test("flushes and closes a supported directory handle", async () => {
    ...
    await fsyncProjectDirectory("/project/.convax", { openDirectory, platform: "posix" })
  ...
  test("rejects Windows fsync EPERM as unavailable instead of claiming durability", async () => {
    ...
    const result = fsyncProjectDirectory("C:\\project\\.convax", {
      platform: "win32",
    ...
    await expect(result).rejects.toBeInstanceOf(NodeDirectoryDurabilityUnavailableError)
    await result.catch((error) => expect(isNodeDirectoryDurabilityUnavailable(error)).toBe(true))
    expect(close).toHaveBeenCalledTimes(1)
  })
  ...
  test("propagates open, non-capability sync and close failures with their exact causes", async () => {
    ...
    await expect(fsyncProjectDirectory("C:\\state", {
      platform: "win32",
    ...
    await expect(fsyncProjectDirectory("/state", {
      platform: "posix",
  ...
  test("does not call the Windows capability gap on another platform", async () => {
    const denied = Object.assign(new Error("operation not permitted"), { code: "EPERM", syscall: "fsync" })
    const result = fsyncProjectDirectory("/state", {
      platform: "posix",
      openDirectory: async () => ({ close: async () => undefined, sync: async () => Promise.reject(denied) }),
    })
    await expect(result).rejects.toBeInstanceOf(NodeDirectoryDurabilityError)
    await expect(result).rejects.not.toBeInstanceOf(NodeDirectoryDurabilityUnavailableError)
  ...
  test.skipIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      try {
        await expect((async () => {
          await fsyncProjectDirectory(root)
          claimedDurable = true
        })()).rejects.toBeInstanceOf(NodeDirectoryDurabilityUnavailableError)
