...
import { ProjectIndexFileMaterializer } from "./project-index-file-materializer"

const projectId = `project_${"1".repeat(32)}` as ProjectId
const projectEpoch = "2".repeat(32) as ProjectIndexResourceReference["projectEpoch"]
...
describe.skipIf(process.platform === "win32")("ProjectIndexFileMaterializer real-filesystem durability", () => {
  ...
  test("does not silently replace unsubmitted native edits when a remote winner arrives", async () => {
    ...
    const materializer = await ProjectIndexFileMaterializer.open({ projectId, projectRoot, projection: state.port, blobs: blobPort(bytes) })
...
function projection(initial: ProjectIndexFileMaterializationPlan) {
  let current = initial
  return {
    port: {
...
function plan(entries: ProjectIndexFileMaterializationPlan["entries"]): ProjectIndexFileMaterializationPlan {
...
function reference(bytes: Uint8Array, id: number): ProjectIndexResourceReference {
  ...
  return {
    ...
    blob: { format: "convax.blob-ref", algorithm: "sha256", digest: blobDigest, byteLength: String(bytes.byteLength) as never, mime: "text/markdown" },
...
function blobPort(available: Map<Digest, Uint8Array>) {
  return {
    async copyVerifiedBytesTo(reference: ProjectIndexResourceReference, stagingPath: string) {
